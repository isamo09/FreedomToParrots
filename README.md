...
На любой платформе первым делом создайте для программы **отдельную папку**
и переместите (или сразу скачайте) исполняемый файл в неё — не запускайте
его прямо из «Загрузок»: рядом с ним появятся настройки, ключи и логи, и
удобнее, когда это всё в одном месте:

```sh
mkdir FreedomToParrots && cd FreedomToParrots
```

**Windows** — скачайте `.exe`, запустите двойным кликом *из этой папки*
(SmartScreen на неподписанный exe — «Подробнее → Выполнить в любом
случае»). Окно консоли называется и выглядит как Freedom To Parrots
(заголовок и значок — из самого exe). Крестик на окне не закрывает
программу сразу: первый клик покажет предупреждение и ничего не остановит,
второй клик (в течение нескольких секунд) — закроет и корректно остановит
все туннели; чтобы просто оставить работать в фоне — сверните окно вместо
закрытия.
...
Серверу в любом случае нужен стабильный выход в интернет (обычного
домашнего Wi-Fi роутера достаточно), и он должен быть включён и запущен
всё то время, пока к нему подключаются клиенты — выключенный или уснувший
компьютер с сервером туннели не поднимет.
...
При запуске (и затем раз в 12 часов) программа сама проверяет
[Releases](https://github.com/isamo09/FreedomToParrots/releases) на GitHub.
Ничего не устанавливается без явного согласия: если есть версия новее,
баннер появится и в консоли, и в веб-панели. Обновить можно кнопкой
«Обновить и перезапустить» в панели, либо набрав `u` и Enter прямо в
консоли. Перед установкой скачанный файл сверяется по SHA256 с суммой из
релиза — если она недоступна или не совпадает, обновление не
устанавливается.
...
internal/procio/           учёт трафика по ОС (точно — только Linux, на остальных n/a)
internal/security/         случайный пароль/токены, сравнение без тайминг-утечек
internal/update/           проверка релизов на GitHub, самообновление с подтверждением и проверкой SHA256
...
- Учёт трафика по процессу точный только на Linux (`/proc/<pid>/io`). На
  Windows это тоже было в первых версиях (через `GetProcessIoCounters`), но
  на практике этот счётчик у Windows надёжно отражает файловый/дисковый
  ввод-вывод и не отражает сетевой — трафик реальных WebRTC-звонков через
  него не рос вообще, значение застывало на первых байтах чтения
  конфига. Показывать заведомо неверное число хуже, чем честно писать
  «недоступно» — так теперь и на Windows, и на macOS/BSD. Сессии при этом
  работают полностью, лимит трафика на этих ОС выставить нельзя.

## Авторство

Ядро туннеля (`cmd/olcrtc`) разрабатывает **zarazaex**,
[openlibrecommunity/olcrtc](https://github.com/openlibrecommunity/olcrtc) —
весь протокол, транспорты и провайдеры реализованы там, этот репозиторий
их не меняет и не форкает, только собирает и встраивает.

GUI-клиент **olcBOX** — независимый сторонний проект,
[alananisimov/olcbox](https://github.com/alananisimov/olcbox), его автор к
Freedom To Parrots отношения не имеет; сборки лежат в релизах этого
репозитория только для удобства — чтобы не искать их отдельно.

Freedom To Parrots (эта консоль, панель и сборка в один файл) — обвязка
вокруг чужого ядра, а не самостоятельный протокол.

## Дисклеймер

Этот репозиторий публикует программу как есть, в исследовательских и
образовательных целях. Всё, что описано выше (установка, запуск,
использование) — исключительно добровольное решение того, кто это делает:
автор репозитория никого не призывает обходить какие-либо ограничения,
блокировки или правила сервисов-провайдеров (Jitsi, Яндекс Телемост,
WbStream) и не несёт ответственности за то, как и для чего инструмент
используется. Соблюдение законодательства своей страны и условий
использования соответствующих сервисов — зона ответственности того, кто
запускает программу.
