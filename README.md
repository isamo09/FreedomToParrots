...
неподписанный exe — «Подробнее → Выполнить в любом случае»). Окно консоли
называется и выглядит как Freedom To Parrots (заголовок и значок — из
самого exe). Крестик на окне не закрывает программу сразу: первый клик
покажет предупреждение и ничего не остановит, второй клик (в течение
нескольких секунд) — закроет и корректно остановит все туннели; чтобы
просто оставить работать в фоне — сверните окно вместо закрытия.
